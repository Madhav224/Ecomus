...
class BannerController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            foreach ($tbody_data as $key => $data) {
                ...
                $actionButtons = '';
                $actionButtons .= $editpermission ? '<a href="javascript:void(0);" class="avatar avatar-status bg-light-primary openmodal-BlogModel" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Edit" recordof="' . encrypt_to('banners') . '/' . encrypt_to($data->id) . '" >
                ...
                $actionButtons .= $deletepermission ? '<a href="javascript:void(0);" class="avatar avatar-status ms-25 bg-light-danger delete_record" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Delete" deleteto="' . 'banner/' . encrypt_to($data->id) . '">
                    <span class="avatar-content">
                        <span class="avatar-content">
                ...
                $actionButtons .= $statuspermission ? '<div class="datatable-switch form-check form-switch form-check-primary ms-25 d-inline-block align-middle"  data-bs-toggle="tooltip" data-bs-placement="top" title="Click to ' . ($data->status == 'active' ? 'Deactivate' : 'Activate') . '">
                    <input type="checkbox" class="form-check-input change_status" id="StatusSwitch' . $key . '" ' . ($data->status == 'active' ? 'checked' : '') . ' statusto="' . encrypt_to('banners') . '/' . encrypt_to($data->id) . '/' . encrypt_to('status') . '"/>
        ...
        $file['breadcrumbs'] = [
            ...
            ['name' => ucwords("Banner list")],
        ];
        $file['title'] = ucwords("Banner list");
        ...
        $file['BannerDataFilterData'] = [
            'name' => 'banner',
            'action' => route('banner.index'),
            'bulk_action_url' => 'banner',
        ...
        $file['BannerFormData'] = [
            ...
            'method' => 'post',
            'submit' => '<i data-feather="save"></i> Save Banner',
            'btnGrid' => 3,
            'no_submit' => false,
            'fieldData' => [
        ...
        return view('banner.banner', $file);
    }
    ...
    public function store(Request $request)
    {
        ...
        $responseData = $brand
            ? ['success' => true, 'Status' => 200, 'Message' => 'The Banner been saved successfully.', 'Redirect' => url()->previous()]
            : ['success' => false, 'Status' => 500, 'Message' => 'The Banner could not be saved.'];
    ...
    public function bulkAction(Request $request)
    {
        ...
        $affected = Banner::whereIN('id', $ids);

        if ($action == "delete") {
            $blogs = Banner::whereIN('id', $ids)->get();
