<div>
        ...
        <section class="flat-spacing-11">
            <div class="container">
                ...
                <div class="tf-cart-countdown">
                    <div class="title-left">
                        ...
                        <p>These products are limited,<span class="text-danger"> Checkout Fast</span> </p>
                    </div>
                ...
                @endphp
                <a href="{{ route('product.detail', $child->product->product_slug) }}" class="img-box">
                    <img src="{{ $main_image }}"
                         alt="{{ $product->product_name }}"
                       >
                </a>
                <div class="cart-info">
                    <a href="{{ route('product.detail', $child->product->product_slug) }}" 
                       class="cart-title link">
                        {{ $child->product->product_name }}
                    </a>
                    <div class="cart-meta-variant">
                        {{ $child->productVariant?->variant_name ?? 'Default' }}
                    </div>
                    {{-- <button wire:click="removeItem({{ $child->id }})" 
                            class="remove-cart link remove">
                        Remove
                    </button> --}}
                    <span wire:click="removeItem({{ $child->id }})" class="remove-cart link remove" >Remove</span>
                </div>
            </td>
            ...
            <td class="tf-cart-item_quantity" >
                <div class="cart-quantity">
                    <div class="wg-quantity flex items-center">
                        <button type="button" wire:click="decrementQty({{ $child->id }})" 
                                class="btn-quantity btndecrease">-</button>
                        <input type="text" value="{{ $child->qty }}" readonly>
                        <button type="button" wire:click="incrementQty({{ $child->id }})" 
                                class="btn-quantity btnincrease">+</button>
                    </div>
                </div>
            </td>

            <td class="tf-cart-item_total tf-variant-item-total" >
                <div class="cart-total ">
                    ₹{{ number_format($child->total_price, 2) }}
                </div>
            </td>
    ...
    <td class="tf-cart-item_quantity">
        <div class="cart-quantity flex items-center">
            <button type="button" wire:click="decrementQty({{ $child->id }})" class="btn-quantity btndecrease">-</button>
            <input type="text" value="{{ $child->qty }}" readonly class="text-center w-12">
            <button type="button" wire:click="incrementQty({{ $child->id }})" class="btn-quantity btnincrease">+</button>
        </div>
    </td>
                            ...
                            <div class="tf-page-cart-checkout">
                                ...
                                <div class="cart-checkbox">
                                    ...
                                    <label for="cart-gift-checkbox" class="fw-4">
                                        <span>Do you want a gift wrap?</span> Only <span class="fw-5">₹50.00</span>
                                    </label>
                                </div>
                                ...
                                <div class="cart-checkbox">
                                    ...
                                    <label for="check-agree" class="fw-4">
                                        I agree with the <a href="{{route('terms-conditions')}}">terms and conditions</a>
                                    </label>
                                </div>
                                <div class="cart-checkout-btn">
                                    ...
                                    @else
                                        <button class="tf-btn w-100 btn-fill animate-hover-btn radius-3 justify-content-center" onclick="alert('Your cart is empty. Please add items to proceed to checkout.')">
                                            <span>Check out</span>
                                        </button>
                                    @endif
                                </div>
