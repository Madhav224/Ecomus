...
$(".open-my-model").click(function () {
    var mymodel = $(this).attr('mymodel');
    $('.error', $("#" + mymodel)).html('');
    $("#" + mymodel).modal("show");
});
...
$("form").submit(function (e) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data?.Status == 403) {
                toastr.error('👋' + (data?.Message ? data?.Message : 'Somthing Wrong!'), 'Error!', { closeButton: true, tapToDismiss: false });
            ...
            if (error.status === 422) {
                ...
                for (var key in errors) {
                    if (errors[key]?.length > 0) {
                        ...
                        var selecttor = $('[name="' + key + '"]', form).length > 1 ? $('[name="' + key + '"]', form)[0] : $('[name="' + key + '"]', form);
                        selecttor = selecttor.length == 0 ? ($('[name="' + key + '[]"]', form).length > 1 ? $('[name="' + key + '[]"]', form)[0] : $('[name="' + key + '[]"]', form)) : selecttor;
